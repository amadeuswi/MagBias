...
class GalaxyHIExperiment:
    ...
    def __init__(
            self,
            redshift: Tuple,
            radio_instrument: dict,
            galaxy_instrument: dict,
            magnification_config=MagnificationConfig()):
    ...
    def _get_ell_tab(self):
        if self.config_class.conservative_fg_forecast:
            lmin_multiplier = self.config_class.conservative_fg_forecast_n_bin_discard
        else:
            lmin_multiplier = 0
        ell_start = self.lmin + lmin_multiplier * self.lmin
        n_ell = (self.lmax - ell_start)//self.delta_ell
        result, new_delta_ell = np.linspace(ell_start, self.lmax, n_ell, retstep = True)
        # result = np.arange(ell_start, self.lmax + self.delta_ell, self.delta_ell)
        print('ell table goes from {} to {}, with new delta ell {} and old one {}'.format(
            result[0], result[-1], new_delta_ell, self.delta_ell))
        self.delta_ell = new_delta_ell
        return result
    ...
    def _get_mag_bin_edges(self):
        res = np.arange(
            self.config_class.min_magnitude,
            self.config_class.max_magnitude +
            self.config_class.delta_mag,
            self.config_class.delta_mag)
    ...
    def _single_dish_noise_wrapper(self):
        return noise_cls_single_dish(self.ell_tab, self.mean_frequency,
                                     self.radio_instrument, 256) * np.ones(self.n_ell)
